...
export const scrapeJobStreet = async (
  req: Request,
  res: Response,
): Promise<Response> => {
  const {
  ...
  if (!COUNTRY_URLS[countryCode]) {
    return ResponseHelper.failure(res, `Invalid country code: ${country}`, 400);
  }
  ...
  try {
    ...
    // Get total jobs
    const $totalJobs = $('#SearchSummary[data-automation="totalJobsMessage"]');
    const totalJobs =
    ...
    jobCards.each((_, element) => {
      ...
      const domain = COUNTRY_URLS[countryCode].split("/")[2];
    ...
    if ($pagination.length) {
      // Find all page numbers using regex pattern
      const $pageLinks = $pagination.find("a");
      const pageNumbers: number[] = [];

      $pageLinks.each((_, el) => {
        const dataAutomation = $(el).attr("data-automation");
        if (dataAutomation && /page-\d+/.test(dataAutomation)) {
    ...
    if ($suggestionSpan.length) {
      const $suggestionLinks = $suggestionSpan.find("a");
      $suggestionLinks.each((_, el) => {
        const $link = $(el);
        const dataAutomation = $link.attr("data-automation");

        // Check if data-automation contains didYouMeanLocation
        if (dataAutomation && /didYouMeanLocation\d+/.test(dataAutomation)) {
